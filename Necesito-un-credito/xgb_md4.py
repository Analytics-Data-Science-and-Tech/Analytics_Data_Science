...
XGBoost_md = XGBClassifier(colsample_bytree = 0.8, gamma = 0.3, learning_rate = 0.01, max_depth = 9, min_child_weight = 15, n_estimators = 300, subsample = 0.8).fit(X, Y)
...
xgb_label_test = np.where(xgb_pred_test < 0.08, 0, 1)

## Data-frame for submission
data_out = pd.DataFrame({'Id': test_id, 'SeriousDlqin2yrs': xgb_label_test})
data_out.to_csv('xgb_submission_md6.csv', index = False)
