...
# man_train = man_train[~(man_test['Seed1'].isnull() | man_test['Seed2'].isnull())].reset_index(drop = True)

woman_train['target'] = np.where(woman_train['ResultDiff'] > 0, 1, 0)
# woman_train = woman_train[~(woman_test['Seed1'].isnull() | woman_test['Seed2'].isnull())].reset_index(drop = True)
...
class Objective:
    ...
    def __call__(self, trial):
        ...
        for i in range(2016, 2022):
            ...
            train_data = womman_train[man_train['Season'] <= i].reset_index(drop = True) 
    
            if ((i + 1) == 2020): 
                continue 
            else:
                test_data = womman_train[man_train['Season'] == (i + 1)].reset_index(drop = True)
    
            X_train = train_data[to_select]
